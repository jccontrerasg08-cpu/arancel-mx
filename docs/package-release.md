...
`arancel-mx==0.2.0` is published on PyPI. The checkout declares `project.version` `0.3.3`; that version is not on PyPI until `pkg-v0.3.3` passes TestPyPI and the OS/Python matrix.
...
The Python distribution uses PEP 440 package versions such as `0.2.0` (PyPI) and in-tree `0.3.3`. Tariff datasets use immutable date tags such as `data-2026.08.15` (`/releases/latest` currently). The PyPI project page long description is frozen at the `0.2.0` upload until `pkg-v0.3.3`.
