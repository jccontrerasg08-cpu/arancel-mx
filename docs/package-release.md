...
`arancel-mx==0.2.0` is published on PyPI. The checkout declares `project.version` `0.3.5`; that version is not on PyPI until `pkg-v0.3.5` passes TestPyPI and the OS/Python matrix. `0.3.3` and `0.3.4` exist only on TestPyPI and must not be promoted or rebuilt under the same versions: the former failed because the prior workflow interpreted `$GITHUB_WORKSPACE` with PowerShell, while the latter reached TestPyPI but its external Windows matrix could not print a Unicode arrow from CLI help using CP1252.
...
The Python distribution uses PEP 440 package versions such as `0.2.0` (PyPI) and in-tree `0.3.5`. Tariff datasets use immutable date tags such as `data-2026.08.17` (`/releases/latest` currently). The PyPI project page long description remains frozen at the `0.2.0` upload until `pkg-v0.3.5` completes its release gates.
