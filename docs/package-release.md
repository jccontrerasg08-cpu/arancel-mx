...
`arancel-mx==0.2.0` is published on PyPI. The checkout declares `project.version` `0.3.6`; that version is not on PyPI until `pkg-v0.3.6` passes TestPyPI and the OS/Python matrix. `0.3.3`, `0.3.4`, and `0.3.5` exist only on TestPyPI and must not be promoted or rebuilt under the same versions: the former failed because the prior workflow interpreted `$GITHUB_WORKSPACE` with PowerShell, `0.3.4` could not print a Unicode arrow from CLI help using CP1252, and `0.3.5` was certified before later Central Hub assets and API changes merged into main.
...
The Python distribution uses PEP 440 package versions such as `0.2.0` (PyPI) and in-tree `0.3.6`. Tariff datasets use immutable date tags such as `data-2026.08.17` (`/releases/latest` currently). The PyPI project page long description remains frozen at the `0.2.0` upload until `pkg-v0.3.6` completes its release gates.
