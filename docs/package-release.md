...
`arancel-mx==0.2.0` is published on PyPI. The checkout declares `project.version` `0.3.4`; that version is not on PyPI until `pkg-v0.3.4` passes TestPyPI and the OS/Python matrix. `0.3.3` exists only on TestPyPI: its external Windows matrix failed before PyPI approval because the prior workflow interpreted `$GITHUB_WORKSPACE` with PowerShell. It must not be promoted or rebuilt under the same version.
...
The Python distribution uses PEP 440 package versions such as `0.2.0` (PyPI) and in-tree `0.3.4`. Tariff datasets use immutable date tags such as `data-2026.08.17` (`/releases/latest` currently). The PyPI project page long description remains frozen at the `0.2.0` upload until `pkg-v0.3.4` completes its release gates.
