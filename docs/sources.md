...
## Apoyo WCO (no es fuente oficial)

Los PDF de nomenclatura HS 2022 de la WCO pueden guardarse en una caché local opcional de **apoyo de lectura**. No son identidad legal mexicana, no entran a DuckDB y **no** se registran en `source_registry.json`. El copyright permanece en la WCO. Cítalos como apoyo; nunca como autoridad LIGIE/NICO.
...
El parser del ledger distingue `last_law_reform` (reforma de la ley) de `latest_tariff_modification` (decreto de fracciones). La release pública `data-2026.08.15` registra `law_reform` 2025-12-29 (`LIGIE_2022_ref02_29dic25.pdf`) y `tariff_decree` 2026-04-23 (`LIGIE_2022_tarifa15_23abr26.pdf`). El fixture del parser espera las mismas fechas. No se copian de visores de terceros.
