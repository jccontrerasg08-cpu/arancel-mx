...
- **package version**: the Python distribution and CLI implementation, reported by `arancel-mx --version`. PyPI currently publishes `0.2.0`. The checkout declares `0.3.4`; the prior `0.3.3` candidate remains TestPyPI-only after a failed external certification matrix and is not eligible for PyPI promotion.
