...
CI and the manual certification workflow exercise the same wheel and sdist smoke boundary.
...
The independent certification layer checks the exact asset set, checksums, source-archive safety and identities, and logical CSV/JSON equivalence. DuckDB has its own consumer-contract and minimum-version probe.
...
Repository CI additionally executes this probe in an isolated DuckDB `1.1.0` environment.
...
A certification workflow PR must also show that it is based on current protected `main`, unstable API/action assumptions were rechecked against official documentation, least-privilege permissions remain intact, Actions stay full-SHA pinned, no credentials/generated production datasets are committed, cleanup fails closed, and live mutation occurs only after merge from trusted `main`.
...
2. Check Releases, Tags, and Issues independently through GitHub.
...
It does **not** by itself prove that an official tariff-source update exists, that legal reconciliation will pass for a future source change, that a future production release will be immutable, that future artifact attestations will verify, or that every external official source is currently reachable.
