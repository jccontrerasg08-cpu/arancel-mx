...
`arancel-mx==0.2.0` está publicado en PyPI (carga del 2026-08-12 vía Trusted Publishing). La matriz externa de SO/Python no fue un gate bloqueante de esa carga. `0.3.2` bloquea PyPI con Ubuntu/Windows/macOS × CPython 3.11–3.13 después de TestPyPI. La guía canónica para aplicaciones aguas abajo es [`docs/external-consumption.md`](docs/external-consumption.md).
...
| Publicación en PyPI | Publicado: `arancel-mx==0.2.0` (`0.3.2` in-tree, no en PyPI hasta `pkg-v0.3.2`) |
