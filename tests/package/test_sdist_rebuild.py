...
def test_sdist_rebuilds_wheel_outside_checkout_and_rebuilt_wheel_passes_probe(
    ...
    sdist = next(dist.glob("arancel_mx-0.2.0.tar.gz"))
    ...
    rebuilt = next(wheels.glob("arancel_mx-0.2.0-py3-none-any.whl"))
...
def test_sdist_can_be_installed_directly_by_clean_certifier(tmp_path: Path) -> None:
    ...
    sdist = next(dist.glob("arancel_mx-0.2.0.tar.gz"))
    completed = subprocess.run(
        [sys.executable, str(CERTIFIER), str(sdist), "--expected-version", "0.2.0"],
        cwd=tmp_path,
        env=_clean_env(),
        text=True,
        capture_output=True,
